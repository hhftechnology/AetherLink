...
class AetherLink:
            ...
            "handle": [
                {
                    ...
                    "upstreams": [{"dial": f"127.0.0.1:{self.local_port}"}],
                    "health_checks": {
                        "active": {
                            "interval": "10s",
                            "timeout": "5s",
    ...
    def wait_for_local_service(self) -> bool:
        ...
        return self.wait_for_service(
            "127.0.0.1", int(self.local_port), service_name="Local service"
        )
    ...
    def create_tunnel(self) -> bool:
        ...
        self.logger.info("Creating tunnel for %s:%s", self.host, self.port)
